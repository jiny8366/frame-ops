...
def fo_page_link(path: str, label: str, icon: str = "") -> None:
    """st.page_link() 안전 래퍼 — 페이지 레지스트리 오류 시 HTML 링크로 폴백."""
    import streamlit as st

    # 1차: 정상 st.page_link 시도
    try:
        if icon:
    ...
    # 2차: URL 기반 HTML 링크 폴백
    # pages/01_상품등록.py → /상품등록
    stem = _Path(path).stem                        # "01_상품등록"
    name = _re.sub(r"^\d+_", "", stem)             # "상품등록"
    url = "/" + _urlparse.quote(name)
    display = f"{icon} {label}".strip() if icon else label
    st.markdown(
        f'<a href="{url}" target="_self" '
        f'style="text-decoration:none;color:inherit;">{display}</a>',
        ...
        with st.popover("☰", help="전체 화면·다른 서비스"):
            ...
            for sec, links in MENU_SECTIONS:
                ...
                for path, lab, icon in links:
                    try:
                        fo_page_link(path, label=lab, icon=icon)
                    except BaseException:
                        pass
            st.divider()
            try:
...
def _render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        if mode is None:
            try:
                fo_page_link("pages/00_서비스선택.py", label="서비스 선택", icon="🔀")
            except BaseException:
                pass

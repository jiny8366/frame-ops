...
export async function PATCH(
  ...
  try {
    ...
    // 지점 매니저는 판매사/직원만 편집. 대상의 현재 role + 새 role 모두 검사.
    if (callerIsManager) {
      const db = getDB();
      ...
      if (
        ...
        return NextResponse.json(
          { data: null, error: '지점 매니저는 판매사/직원 계정만 수정할 수 있습니다.' },
          { status: 403 }
        );
      }
      if (
        ...
        return NextResponse.json(
          { data: null, error: '지점 매니저는 판매사/직원 외 역할로 변경할 수 없습니다.' },
          { status: 403 }
        );
      }
      // store_id 변경도 차단 (본인 매장 외부로 이전 불가)
